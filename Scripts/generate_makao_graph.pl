...
my $autoconf_assignments_available=0;
...
GetOptions (
  ...
  'debugperf' => \$debugperf,
  'in:s' => \$file_in,
  'assign:s' => \$file_assign,
  'out:s' => \$file_out,
  'xml:s' => \$xml_out,
  'format:s' => \$format,
  'unused' => \$unused,
  'explicit' => \$hide_implicit
#  'dir:s' => \$build_dir
) or usage_and_exit ();
...
#  ($build_dir eq $current_dirs[$current_dir_counter]) or die "Wrong initial build directory.";
  ...
  if($file_assign) {
    print STDERR "Found autoconf assignment file!\n";
    $autoconf_assignments_available=1;
    open ($HANDLE_ASSIGN, $file_assign) or die "Assignment file ${file_assign} could not be found!";
  }
}
...
my %autoconf_makefile_assignments=();
my %autoconf_source_assignments=();
...
my $TARGETS_USER_TIMINGS=12;#string representing total user timing (i.e., including timing of recursive make calls), like this ",1.00,83.00,2:98.00"
my $TARGETS_SYSTEM_TIMINGS=13;#string representing total system timing (i.e., including timing of recursive make calls), like this ",1.00,83.00,2:98.00"
my $TARGETS_ELAPSED_TIMINGS=14;#string representing total elapsed timing (i.e., including timing of recursive make calls), like this ",1.00,83.00,2:98.00"
my $TARGETS_USER_NONREC_TIMINGS=15;#string representing user timing of own build activity (not recursive make calls), like this ",1.00,83.00,2:98.00"
my $TARGETS_SYSTEM_NONREC_TIMINGS=16;#string representing system timing of own build activity (not recursive make calls), like this ",1.00,83.00,2:98.00"
my $TARGETS_ELAPSED_NONREC_TIMINGS=17;#string representing elapsed timing of own build activity (not recursive make calls), like this ",1.00,83.00,2:98.00"
my $TARGETS_MACROS=18;
...
if($autoconf_assignments_available){
...
if(consume("\\s*Re\-executing(\\[\\d+\\])?:\\s+make.*")){
  $line_count++;
  die "Sorry, this is a strange build: there are actually two builds in this single trace. The second build starts on line ${line_count}. Please try to split the file in two parts and parse one part at a time.\n\nIf you want to analyze to first part of the trace file, you will first need to search the last line starting with \"# make: Leaving directory\" and copy it right before line ${line_count}.\n\nIf you want to analyze the second part of the trace file, you will first need to search the first line starting with \"# make: Entering directory\" and copy it right after the first occurrence of the line \"Reading makefiles...\" following line ${line_count}. "
}
...
(${nr_of_make_processes_started} == ${nr_of_dirs}) or die "${nr_of_make_processes_started} make processes started, but ${nr_of_dirs} current dirs found...";
#resolve_duplicates();
...
#      $four=1;
      if($debug){
	print STDOUT "Detected GNU Make 4.x.\n";
      }
    }else{
      $back_quote="`";
      if($debug){
...
sub collect_current_dirs {
    ...
    my $index=0;#necessary to assign directory name to make process in right order, since it only occurs in DB at end of trace
    my @index_stack=();#necessary to assign directory name to make process in right order, since it only occurs in DB at end of trace
    my @stack=();
...
#    my $nr_of_make_processes_without_entering_or_leaving_info=0;
    ...
    do{
      if(consume_banner()){
	if($debug){ print STDOUT "ENTERED new Make process on line ${line_count}\n"; }
...
#	}
	...
	if(@index_stack == 0){
	  die "Parsing ${file_in}...\nSorry, this is a strange build: there are actually two builds in this single trace, because one of the files included in the makefile were modified or created during the build. The second (actual) build starts after line ${line_count}.\n\nPlease try to split the file.";
	}else{
	  my $ind=pop(@index_stack);
	  $current_dirs_ref->[$ind]=$dir_name;
	}
	#push(@{$current_dirs_ref},$dir_name);
	#	print STDERR "Found ${dir_name} for ${input} at ${line_count}\n";
	if($debug){ print STDOUT "ENTERED ${dir_name} on line ${line_count}\n"; }

#	$entered_new_make_process=0;
      }elsif($input =~ /\s*# Finished Make data base on.*/){
#	if($entered_new_make_process){
...
#	  my $top_of_stack=$stack[$#stack];
#	  push(@{$current_dirs_ref},$top_of_stack);;#do NOT push on stack => regular stack check not disturbed
##	  print STDERR "Left ${top_of_stack} for ${input} at ${line_count}\n";
#
#	  $entered_new_make_process=0;
#	}
	...
	if(@stack == 0){
	  ...
	  my $check=pop(@stack);
	  if($debug){ print STDOUT "EXITED ${check} on line ${line_count}\n"; }
	}
...
sub collect_current_dirs_OLD_VERSION_WITHOUT_CURDIR {
    my ($current_dirs_ref)=@_;
...
#    print STDOUT "${line_count}: +++++++++++++++++++\n";
	...
	#QT_Windows.log issue with missing Considering target file
	consume_and_capture("\\s*Considering target file ${back_quote}(.*)'.*",\@captures) or consume_and_capture("\\s*File ${back_quote}(.*)' does not exist.*",\@captures) or consume_and_capture("\\s*Pruning file ${back_quote}(.*)'.*",\@captures);
...
#	    $new_target=register_target($captures[0],\%subtargetsnametoid,\@makefiles,$current_dir);
	...
	#QT_Windows.log issue with missing Considering target file
	if($input =~ /\s*Considering target file ${back_quote}\Q${new_target_name}\E'.*/ or $input =~ /\s*File ${back_quote}\Q${new_target_name}\E' does not exist.*/){
	...
	until(consume("\\s*# Make data base, printed on.*")){
	    @captures=();
	    consume_and_capture("\\s*Considering target file ${back_quote}(.*)'.*",\@captures) or consume_and_capture("\\s*File ${back_quote}(.*)' does not exist.*",\@captures) or consume_and_capture("\\s*Pruning file ${back_quote}(.*)'.*",\@captures) or consume("\\s*(?:M|m)ake(?:\\[.*\\])?: Nothing to be done for ${back_quote}\Q${new_target_name}\E'.*") or die "Expected another top-level target on line ${line_count}, but found: ${input}\n";
	    if($input =~ /\s*Pruning.*/){
...
sub read_makefiles {
	...
	if(consume("^\\s*(?:M|m)ake(?:\\[.*\\])?:\\s+\\/bin\\/bash -c: [cC]ommand not found")){ #Firefox hack
	  consume("#\\s*(?:M|m)ake(?:\\[.*\\])?: Entering directory ${back_quote}.*'.*");
	    ...
	    if(consume("^Makefile:\\d+: \\*\\*\\* commands commence before first target\. +Stop")){
	      $error_encountered=1;
	      last;#emacs 19.34 error
	    }
	}
...
sub update_makefiles {
    ...
    #QT_Windows.log issue with missing Considering target file
    my @captures=();
    while(consume_and_capture("\\s*Considering target file ${back_quote}(.*)'.*",\@captures) or consume_and_capture("\\s*File ${back_quote}(.*)' does not exist.*",\@captures) or consume_and_capture("\\s*Pruning file ${back_quote}(.*)'.*",\@captures)){
...
#	register_edge($parent_target,$new_target);
	...
	#QT_Windows.log issue with missing Considering target file
	unless($input =~ /\s*Pruning.*/){#not needed when pruning
...
#    until(${error} or consume("\\s*Finished prerequisites of target file ${back_quote}\Q${parent_target_name}\E'.*") or consume("\\s*File ${back_quote}\Q${parent_target_name}\E' was considered already.*")){
    until(${error} or consume("\\s*Finished prerequisites of target file ${back_quote}\Q${parent_target_name}\E'.*")){
      if(consume("\\s*File ${back_quote}\Q${parent_target_name}\E' does not exist.*")){
	#do not care
	...
	return 1;
      }elsif(consume_and_capture("\\s*Considering target file ${back_quote}(.*)'.*",\@captures) or consume_and_capture("\\s*File ${back_quote}(.*)' does not exist.*",\@captures)){
...
#	      print STDERR "TIMER set to ${explicit_shell_command_timer_running}\n";
		...
		my $user_time=$captures[0];
		my $system_time=$captures[1];
		my $elapsed_time=$captures[2];

		if($debugperf){
		  print STDERR "... and used for ${parent_target}: ${user_time},${system_time},${elapsed_time}\n";
		}

		if($debug){
		  print STDERR "${parent_target}: ${user_time},${system_time},${elapsed_time}\n";
	        }

		my $parent_target=$subtargetsnametoid_ref->{$parent_target_name};
		attach_total_timings($parent_target,$user_time,$system_time,$elapsed_time);
		...
		unless($returned_from_recursive_make){# and $really_should_see_performance_measurement_of_recursive_make_now){
		  ...
		  attach_non_recursive_timings($parent_target,$user_time,$system_time,$elapsed_time);
		}else{
#		  if($returned_from_recursive_make){
		  if($debugperf){
...
#		print STDOUT "NEW macro: @{tmp_ar}\n";
	      ...
	      #command line:
	      #     1 command:
	      #          without $SHELL: take first timing information
	      #          with $SHELL: ignore first N timing informations
	      #     >1 command:
	      #          without $SHELL:
	      #                 with recursive make in for-loop: 1 measurement at end, NOT in own
	      #                 other: 1 measurement at end, IN own
	      #          with $SHELL:
	      #                 with recursive make in for-loop: ignore first N timing informations, then 1 measurement at end, NOT in own
	      #                 other: ignore first N timing informations, then 1 measurement at end, IN own

	      #disables the detection of recursive make's performance, for example when recursive make is done in a bash-loop
#	      if($returned_from_recursive_make){
#		print STDERR "<<< DISABLE RETURN 3\n";
#		$returned_from_recursive_make=0;
		#$really_should_see_performance_measurement_of_recursive_make_now=0;
#	      }
...
#		  print STDOUT "THROWING away ${str} due to the star\n";
		  ;
		    ...
		    ;
		}elsif(not($hide_implicit) and ($str =~ /(\S+)\.(\S+)$/) and not $2 =~ /\// and not $str =~ /\\/ and not $2 =~ /^\d+\"?$/ and not $str =~ /\.\.\.+/){
		  $str =~ s/^\"//;#omit leading "
		  $str =~ s/\"$//;#omit trailing "
		  $str =~ s/^\`//;#omit leading `
		  $str =~ s/\`$//;#omit trailing `
...
sub database_strcache{
  if(!consume("\\s*# # of strings in strcache:.*") and !consume("\\s*# strcache buffers:\\s+\\d.*") and !consume("\\s*# No strcache buffers.*")){ return 0; }

  if($input =~ m/No strcache buffers/){
    return 1;
  }
...
sub write_gdf_targets {
  ...
  print $HANDLE_OUT "nodedef> name,localname VARCHAR(255),makefile VARCHAR(255),line INT,style,concern VARCHAR(50),error INT,tstamp INT,phony INT,dir VARCHAR(255),base VARCHAR(255),inuse INT,totalusertime VARCHAR(255),totalsystemtime VARCHAR(255),totalelapsedtime VARCHAR(255),ownusertime VARCHAR(255),ownsystemtime VARCHAR(255),ownelapsedtime VARCHAR(255)\n";
...
#    print $HANDLE_OUT "${key},${data[${TARGETS_NAME}]},${data[${TARGETS_MAKEFILE}]},${data[${TARGETS_LINE}]},${style},${concern},${data[${TARGETS_META}]},${data[${TARGETS_ERROR}]},${data[${TARGETS_TIME}]}\n";
    ...
    my $targets_user_time=obtain_a_timing($key,$TARGETS_USER_TIMINGS);
    my $targets_system_time=obtain_a_timing($key,$TARGETS_SYSTEM_TIMINGS);
    my $targets_elapsed_time=obtain_a_timing($key,$TARGETS_ELAPSED_TIMINGS);

    my $targets_user_nonrec_time=obtain_a_timing($key,$TARGETS_USER_NONREC_TIMINGS);
    my $targets_system_nonrec_time=obtain_a_timing($key,$TARGETS_SYSTEM_NONREC_TIMINGS);
    my $targets_elapsed_nonrec_time=obtain_a_timing($key,$TARGETS_ELAPSED_NONREC_TIMINGS);

    print $HANDLE_OUT "${key},${commaless_name},${targets_makefile},${data[${TARGETS_LINE}]},${style},${concern},${data[${TARGETS_ERROR}]},${data[${TARGETS_TIME}]},${data[${TARGETS_PHONY}]},${targets_dirname},${commaless_base_name},${data[${TARGETS_INUSE}]},${targets_user_time},${targets_system_time},${targets_elapsed_time},${targets_user_nonrec_time},${targets_system_nonrec_time},${targets_elapsed_nonrec_time}\n";
  }
...
#    print $HANDLE_OUT "${key},${data[${TARGETS_NAME}]},${data[${TARGETS_MAKEFILE}]},${data[${TARGETS_LINE}]},${style},${concern},${data[${TARGETS_META}]},${data[${TARGETS_ERROR}]},${data[${TARGETS_TIME}]}\n";
	...
	my $targets_user_time="[]";#by definition
	my $targets_system_time="[]";#by definition
	my $targets_elapsed_time="[]";#by definition
	my $targets_nonrec_user_time="[]";#by definition
	my $targets_nonrec_system_time="[]";#by definition
	my $targets_nonrec_elapsed_time="[]";#by definition

	print $HANDLE_OUT "${new_key},${targets_name},${targets_makefile},${data[${TARGETS_LINE}]},${style},${concern},${data[${TARGETS_ERROR}]},${data[${TARGETS_TIME}]},${data[${TARGETS_PHONY}]},${targets_dirname},${targets_basename},${data[${TARGETS_INUSE}]},${targets_user_time},${targets_system_time},${targets_elapsed_time},${targets_nonrec_user_time},${targets_nonrec_system_time},${targets_nonrec_elapsed_time}\n";
    }
  }
...
sub read_assignment_data {
  while(<$HANDLE_ASSIGN>){
    my $cline=$_;
    chomp($cline);

    if($cline =~ /^S,\"([^,]+)\",\"(.*)\"\s*$/){
      $autoconf_makefile_assignments{$1}=$2;      
    }elsif($cline =~ /^D,\"([^,]+)\",\"(.*)\"\s*$/){
      $autoconf_source_assignments{$1}=$2;
    }else{
      die "Error while reading autoconf assignments: <${cline}>";
    }
  }
}
...
#    $writer->startTag("build", "name" => "${main_target}");
    for my $assign (keys(%autoconf_makefile_assignments)){
      my $val=$autoconf_makefile_assignments{$assign};
      $writer->dataElement("makefile_assignment","${val}","var" => "${assign}");
    }

    for my $assign (keys(%autoconf_source_assignments)){
      my $val=$autoconf_source_assignments{$assign};
      $writer->dataElement("source_assignment","${val}","var" => "${assign}");
    }
...
sub write_prolog {
  ...
  my $system_times = "";
  my $elapsed_times = "";
  my $user_nonrec_times = "";
  my $system_nonrec_times = "";
  my $elapsed_nonrec_times = "";
  ...
  for my $key (keys(%targets)){
    ...
    my $targets_user_time=obtain_a_timing($key,$TARGETS_USER_TIMINGS);
    $targets_user_time =~ s/;/','/g;
    unless($targets_user_time =~ m/^\[\]$/){
      $targets_user_time="'${targets_user_time}'";
    }
    $user_times .= "total_user_time(${key},${targets_user_time}).\n";

    my $targets_system_time=obtain_a_timing($key,$TARGETS_SYSTEM_TIMINGS);
    $targets_system_time =~ s/;/','/g;
    unless($targets_system_time =~ m/^\[\]$/){
      $targets_system_time="'${targets_system_time}'";
    }
    $system_times .= "total_system_time(${key},${targets_system_time}).\n";

    my $targets_elapsed_time=obtain_a_timing($key,$TARGETS_ELAPSED_TIMINGS);
    $targets_elapsed_time =~ s/;/','/g;
    unless($targets_elapsed_time =~ m/^\[\]$/){
      $targets_elapsed_time="'${targets_elapsed_time}'";
    }
    $elapsed_times .= "total_elapsed_time(${key},${targets_elapsed_time}).\n";

    my $targets_user_nonrec_time=obtain_a_timing($key,$TARGETS_USER_NONREC_TIMINGS);
    $targets_user_nonrec_time =~ s/;/','/g;
    unless($targets_user_nonrec_time =~ m/^\[\]$/){
      $targets_user_nonrec_time="'${targets_user_nonrec_time}'";
    }
    $user_nonrec_times .= "own_user_time(${key},${targets_user_nonrec_time}).\n";

    my $targets_system_nonrec_time=obtain_a_timing($key,$TARGETS_SYSTEM_NONREC_TIMINGS);
    $targets_system_nonrec_time =~ s/;/','/g;
    unless($targets_system_nonrec_time =~ m/^\[\]$/){
      $targets_system_nonrec_time="'${targets_system_nonrec_time}'";
    }
    $system_nonrec_times .= "own_system_time(${key},${targets_system_nonrec_time}).\n";

    my $targets_elapsed_nonrec_time=obtain_a_timing($key,$TARGETS_ELAPSED_NONREC_TIMINGS);
    $targets_elapsed_nonrec_time =~ s/;/','/g;
    unless($targets_elapsed_nonrec_time =~ m/^\[\]$/){
      $targets_elapsed_nonrec_time="'${targets_elapsed_nonrec_time}'";
    }
    $elapsed_nonrec_times .= "own_elapsed_time(${key},${targets_elapsed_nonrec_time}).\n";
  ...
  print $HANDLE_OUT ($user_times);
  print $HANDLE_OUT ($system_times);
  print $HANDLE_OUT ($elapsed_times);
  print $HANDLE_OUT ($user_nonrec_times);
  print $HANDLE_OUT ($system_nonrec_times);
  print $HANDLE_OUT ($elapsed_nonrec_times);
...
sub resolve_target_name {
  ...
  if($target_name =~ /^\/.*/){#absolute path can also contain ..
    @path_comps=split(/\//,"${target_name}");
  }else{
    @path_comps=split(/\//,"${current_dir_name}/${target_name}");
  }
...
sub attach_total_timings {
  my ($target,$user,$system,$elapsed)=@_;
  
  attach_a_timing($target,$TARGETS_USER_TIMINGS,$user);
  attach_a_timing($target,$TARGETS_SYSTEM_TIMINGS,$system);
  attach_a_timing($target,$TARGETS_ELAPSED_TIMINGS,$elapsed);
}

sub attach_non_recursive_timings {
  my ($target,$user,$system,$elapsed)=@_;
  
  attach_a_timing($target,$TARGETS_USER_NONREC_TIMINGS,$user);
  attach_a_timing($target,$TARGETS_SYSTEM_NONREC_TIMINGS,$system);
  attach_a_timing($target,$TARGETS_ELAPSED_NONREC_TIMINGS,$elapsed);
}

sub attach_a_timing {
  my ($target,$index,$timing)=@_;

  my $str=";${timing}";
  if(exists($targets{$target}) and exists($targets{$target}->[$index])){
    $str=($targets{$target}->[$index]) . $str;
  }

  $targets{$target}->[$index]=$str;
}
...
#  print STDERR ("  -debugperf               show debug info for performance\n");
  ...
  print STDERR ("  -assign IN           file containing autoconf variable assignments for (make)files and config.h\n");
...
#  print STDERR ("  -dir DIRNAME  directory from within build has been started\n");
  ...
  print STDERR ("  -explicit              ignore implicit dependencies\n");
